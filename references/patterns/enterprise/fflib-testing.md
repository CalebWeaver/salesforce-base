# FFLib Testing Patterns

## Test Data Setup with Unit of Work

Use `Application.UnitOfWork.newInstance()` for all test data DML — the same pattern used in production. Builders create in-memory records, UoW handles insert ordering and relationship wiring.
...
For multi-level hierarchies, `registerNew` with the relationship parameter handles all wiring:
...
## Mocking with ApexMocks

Use `fflib_ApexMocks` for mocking selectors, services, and unit of work in unit tests. Use `Application.*.setMock()` for dependency injection in test context.
...
public class CustomerFixture {
    ...
    // Result classes same as lightweight pattern
}
```

## References

- `references/fflib-apex-mocks/` for full ApexMocks source and additional patterns
